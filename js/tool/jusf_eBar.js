...
!function () {
    var jusfBar = {};
    var globalColors = [
        {start: '#E90102', end: '#E16903'},
        {start: '#E16903', end: '#C38707'},
        {start: '#028FDB', end: '#0CCBEB'},
        //{start: '#1EB5F4', end: '#7CE5FD'},
    ].reverse();

    var globalRGBColors = ['rgba(233,1,2,0.4)', 'rgba(225,105,3,0.4)', 'rgba(2,143,219,0.4)'];
        ...
        var baseOption = {
            ...
            tooltip: {
                ...
                formatter: function (params) {
                    ...
                    hint = hint + ( params.name != null ? params.name : '-') + '\t' + ':' + '\t' + (params.value != null ? params.value : '-');
    ...
    /**
     * 获取序列数据
     *x轴为value类型，y轴为category类型，柱条为水平方向
     * @param seriesData
     * @returns {Array}
     */
    function getHrzBarSeries(seriesData, ec, xMax, params) {
        ...
        if (seriesData != null) {
            ...
            var unit = seriesData[0].unit != null ? seriesData[0].unit : defaultUnit;
            for (var i = 0; i < seriesData.length; i++) {
                    ...
                    var tmpItem = {
                        ...
                        label: {
                            normal: {
                                ...
                                position: 'right',
                                formatter: function (params) {
                                    return seriesData[0].data[params.dataIndex] + unit;
                                },
                                textStyle: {
                                    color: globalAxisTextColor,
                                    fontSize: 13
                ...
                for (var j = 0; j < barData.length; j++) {
                    var start, end;
                    if (!isAnother) {
                        start = (colors[j] != null && colors[j].start != null) ? colors[j].start : ("#" + Math.floor(Math.random() * (256 * 256 * 256 - 1)).toString(16));
                        end = (colors[j] != null && colors[j].end != null) ? colors[j].end : ("#" + Math.floor(Math.random() * (256 * 256 * 256 - 1)).toString(16));
                    } else {
                        start = (colors[j % 2] != null && colors[j % 2].start != null) ? colors[j % 2].start : ("#" + Math.floor(Math.random() * (256 * 256 * 256 - 1)).toString(16));
                        end = (colors[j % 2] != null && colors[j % 2].end != null) ? colors[j % 2].end : ("#" + Math.floor(Math.random() * (256 * 256 * 256 - 1)).toString(16));
                    }
                    var barItem = {
                        value: barData[j],
                        itemStyle: {
    ...
    function getHrzBarOtherSeries(seriesData, ec, xMax, status) {
        ...
        if (seriesData != null) {
            if (seriesData.length == 1 && status != null) {
                colors = globalColors.splice(status - 1, status);
            }
                    ...
                    var tmpItem = {
                        ...
                        label: {
                            normal: {
                                ...
                                position: 'right',
                                formatter: function (params) {
                                    return seriesData[0].data[params.dataIndex] + (seriesData[0].unit != null ? seriesData[0].unit : "件");
                                },
                                textStyle: {
                                    color: globalAxisTextColor,
                                    fontSize: 13
    ...
    function addGraphic(yAxisData, height) {
        var graphic = [];
        var colorsRGB = ['rgba(233,1,2,0.4)', 'rgba(225,105,3,0.4)', 'rgba(2,143,219,0.4)'];
        var colors = ['#E90102', '#E16903', '#028FDB'];
        var rectWidth = 25;

        var num = yAxisData.length;
    ...
    jusfBar.drawHrzBar = function (id, datas) {
        ...
        try {
            ...
            if (myChart != null) {
                ...
                // 设置option
                var option = initHrzBarOption(datas, chart.gECharts, chart.height);
    ...
    jusfBar.drawHrzBarOther = function (id, datas, status) {
        ...
        try {
            ...
            if (myChart != null) {
                ...
                // 设置option
                var option = initHrzBarOtherOption(datas, chart.gECharts, tmpStatus);
