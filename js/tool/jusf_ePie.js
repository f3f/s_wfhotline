...
!function () {
    ...
    function getRingPieRSeries(seriesData, params) {
        ...
        if (params != null) {
            ...
            var subType = params.subType != null ? params.subType : 0;//标识 饼图标签 以及 标线 的颜色，0，根据饼图自适应外环；1，统一颜色
            if (type == 0) {
                defaultRadius = ['32%', '65%'];
                    ...
                    data[k].itemStyle = {
                        normal: {
                            color: new ec.graphic.LinearGradient(0, 0, 0, 1, [{
                                offset: 0, color: '#FBE459' // 0% 处的颜色
                            }, {
                                offset: 1, color: '#E61E1D' // 100% 处的颜色
                            }], false),
                            opacity: 1
                        },
                        emphasis: {
    ...
    function setDataByLinearGradient(seriesData, i, ec, isAnother) {
        var defaultNameColor = seriesData[i].nameColor != null ? seriesData[i].nameColor : '#DD5B19';
        var data = seriesData[i].data;
        for (var k = 0; k < data.length; k++) {
            if (k == 1) {
                data[k].itemStyle = {
                    normal: {
                        color: new ec.graphic.LinearGradient(0, 0, 0, 1, [{
                            offset: 0, color: seriesData[0].colors[1] // 0% 处的颜色
                        }, {
                            offset: 1, color: seriesData[0].colors[0] // 100% 处的颜色
                        }], false),
                        opacity: 1
                    },
                    emphasis: {
    ...
    /**
     * 设置中心提示信息‘55%’
     * @returns {{type: string, radius: string[], center: string[], data: *[], avoidLabelOverlap: boolean, hoverAnimation: boolean, label: {normal: {position: string, textStyle: {color: string, fontSize: number}, formatter: Function}}, itemStyle: {normal: {color: string}, emphasis: {color: string}}, tooltip: {show: boolean}}}
     */
    function setAssistItem(assistData) {
        /**
         * 中心‘55%’提示信息
         * @type {{type: string, radius: string[], center: string[], data: *[], avoidLabelOverlap: boolean, hoverAnimation: boolean, label: {normal: {position: string, textStyle: {color: string, fontSize: number}, formatter: Function}}, itemStyle: {normal: {color: string}, emphasis: {color: string}}, tooltip: {show: boolean}}}
         */
        var item = {
            type: 'pie',
            radius: ['0%', '40%'],
    ...
    /**
     * 设置环形饼图option，样式参照民意预测页面--‘下月民意内容预测’，
     * 标题在饼图下侧
     * @param seriesData
     * @param ec
     */
    ...
    function getRosePieSeries(seriesData) {
        ...
        for (var i = 0; i < seriesData.length; i++) {
            var item = {
                name: seriesData[i].name,
                type: 'pie',
                roseType: seriesData[i].roseType != null ? seriesData[i].roseType : 'radius',
                radius: seriesData[i].radius != null ? seriesData[i].radius
                    : ['25%', '70%'],
                center: seriesData[i].center != null ? seriesData[i].center
                    : ['50%', '55%'],
        ...
        /**
         * 辅助数据item
         * @type {{type: string, radius: string[], center: string[], data: *[], avoidLabelOverlap: boolean, hoverAnimation: boolean, label: {normal: {show: boolean}, emphasis: {show: boolean}}, itemStyle: {normal: {color: string}, emphasis: {shadowBlur: number, shadowOffsetX: number, shadowColor: string}}, tooltip: {show: boolean}}}
         */
        if (series[0].data.length > 0) {
            var assistItem = {
                type: 'pie',
    ...
    function getRingSortPieSeries(seriesData, colors) {
        ...
        for (var i = 0; i < seriesData.length; i++) {
            if (i > 3) {
                ...
                var percent = addedData[0].percent != null ? Number(addedData[0].percent) : 0;
                var addedOneIsShow = (addedData[0].value > 0 && percent > 0) ? true : false
    ...
    function getDoubleRingPieSeries(seriesData, type) {
        ...
        var clockWise = (type != null && (type == 0 || type == '-')) ? false : true;
     ...
     * 设置饼图的基础option
     * @param datas
     * @returns {{backgroundColor: string, title: *[], tooltip: {show: *, trigger: string, borderColor: string, borderWidth: number, padding: number, backgroundColor: string}, legend: {data: Array, show: *, y: string, left: string, x: string, orient: string, textStyle: {fontSize: number, color: string}}, toolbox: {right: string, show: *, feature: {saveAsImage: {show: boolean, iconStyle: {normal: {color: string, borderColor: string, borderWidth: number, opacity: number}, emphasis: {color: string, borderColor: string, borderWidth: number}}}}}, animationDuration: number, animationEasingUpdate: string}}
     */
    function setBaseOption(datas, defaultColors) {
        ...
        var baseOption = {
            ...
            tooltip: {
                ...
                formatter: function (params) {
                    ...
                    hint = hint + params.name + '\t' + ':' + '\t' + params.value + '\t' + '(' + params.percent + '%)';
        ...
        if (datas.colors != null) {
            ...
            var colors = ['#FC3E1D', '#1787FB', '#FEE233', '#FD9526', '#149343', '#1BA4DE', '#28E4FD', '#1FB5FC'];
    ...
    function initDoubleRingPieOption(datas, type) {
        ...
        option.legend = [{
            ...
            top: 'center',
            x: 'left',
            data: (legend[0] != null && legend[0].data != null) ? legend[0].data : (legend.data != null ? legend.data : []),
            show: (legend[0] != null && legend[0].isShow != null) ? legend[0].isShow : (legend.isShow != null ? legend.isShow : true),
            textStyle: {
                fontSize: 12,
                color: '#fff'
            },
            ...
            show: (legend[1] != null && legend[1].isShow != null) ? legend[1].isShow : true,
            textStyle: {
                fontSize: 12,
                color: '#fff'
            },
        ...
        } else if (datas.colors == null && option.legend[0].data != null && option.legend[1].data != null) {
            var colors = [];
            var size;
            if (type != null && (type == 0 || type == '-')) {
                colors = ['#976EFB', '#423691', '#4F5AC2', '#5E659A', '#FC3E1D', '#FD8825', '#FDB92C', '#FCFD37', '#87C826', '#1DBD90', '#0B54B1'];
            } else {
                colors = ['#FC3E1D', '#FA8023', '#FDB92C', '#FCFD37', '#87C826', '#1DBD90', '#28E4FD', '#1FB5FC', '#157CFB', '#1056FB', '#0835CC', '#976EFB', '#423691', '#4F5AC2', '#5E659A'];
            }

            size = colors.length;
            var num;
            if (legend instanceof Array) {
                num = option.legend[0].data.length + option.legend[1].data.length;
            } else {
    ...
    function initRingPieR01Option(datas) {
        var defaultColors = ['#2A52C6', '#2B54CA', '#2B62D2', '#347FE5', '#389BEE'];
    ...
    function initRingPieR02Option(datas) {
        var defaultColors = ['#0078FF', '#FFBA00', '#2A52C6', '#2B54CA', '#2B62D2', '#347FE5', '#389BEE'];
        var option = setBaseOption(datas, defaultColors);
        var series = [];
        series = setOptionParam(series, datas.series);
    ...
    /**
     * 1 以 网络部门热线 -- 本市诉求热点分布  为例
     * 2 以 网络部门热线 -- 民意钻取--民意类型环形图  为例
     * @param datas
     */
    function initRingPieR03Option(datas) {
        var defaultColors = ['#FF502D', '#FF8D27', '#A6D845', '#4AC977', '#09B2E9', '#168FFF', '#1F6EFF', '#2F58ED', '#2A3ED2', '#4A3FB0', '#6F3FB0', '#338CD9'];
        var option = setBaseOption(datas, defaultColors);
        var series = [];
        series = setOptionParam(series, datas.series);
        ...
        try {
            ...
            if (myChart != null) {
                ...
                myChart.on('click', function (params) {
                    // 点击调用,辅助内圈点击无效
                    if (param != null && fn != null && params.seriesIndex == 0) {
    ...
    jusfPie.drawRingPieDoubleAnother = function (id, datas) {
        ...
        try {
            ...
            if (myChart != null) {
                ...
                // 设置option
                var option = initRingPieDoubleAnotherOption(datas, chart.gECharts);
    ...
    /**
     * 以 部门民意-市直机关单位--民意内容环形图  为例
     * 绘制双环形图，扇形区域覆盖
     * @param id
     * @param datas
     * @param type，两种情况：1）type=0或者‘-’，适用场景：受理民意环形图；2）type==undefined或者其他数字,适用场景：民意内容环形图
        ...
        try {
            ...
            if (myChart != null) {
                ...
                myChart.on('click', function (params) {
                     ...
                     */
                    if (subDatas != null && params.seriesIndex == 1 && (type != null && (type == 0 || type == '-'))) {
     ...
     * @param datas
     */
    jusfPie.drawRingPieR01 = function (id, datas) {
        var chart = getChart(id);
        var myChart = chart.myChart;
     ...
     * @param datas
     */
    jusfPie.drawRingPieR02 = function (id, datas) {
        var chart = getChart(id);
        var myChart = chart.myChart;
     ...
     * @param datas
     */
    jusfPie.drawRingPieR03 = function (id, datas) {
        var chart = getChart(id);
        var myChart = chart.myChart;
